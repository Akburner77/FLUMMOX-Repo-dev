...
// Fast path: base64-decode url= param → direct m3u8/mp4 if valid
...
class GogoCdn : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        if (!iframe.isNullOrBlank() && iframe != url) {
            ...
            BCLog.d("GogoCdn recurse → $nested")
    ...
    private fun tryInlineDecode(playerUrl: String): String? {
        ...
        // double base64 (some sites layer it)
        if (one != null) {
            val two = try {
    ...
    private fun emit(url: String, cb: (ExtractorLink) -> Unit) {
        val type = when {
    ...
    private fun emitBlogger(url: String, cb: (ExtractorLink) -> Unit) {
        cb.invoke(
            newExtractorLink(
                source = name,
                name = "$name Blogger",
